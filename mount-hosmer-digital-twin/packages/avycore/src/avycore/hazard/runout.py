"""AvyCore runout simulation: fast routing and an advanced particle ensemble.
...
from .protocols import Grid, Parameters
from .zone import ReleaseZone
...
class RunoutEngine(Protocol):
    ...
    def simulate(
        ...
        *,
        ...
        release_size: str,
        seed: int | None,
        alpha_override_deg: float | None = None,
...
def _flow_regime_factors(config: Parameters, flow_regime: str | None) -> dict[str, float]:
    """Friction and alpha adjustments for a flow regime; identity for the default."""
    identity = {"mu_scale": 1.0, "xi_scale": 1.0, "alpha_shift_deg": 0.0}
    if not flow_regime:
        return identity
    try:
        regimes = config.require("runout.flow_regime")
    except Exception:  # pragma: no cover - configurations predating flow regimes
        return identity
    factors = regimes.get(flow_regime)
    if not factors:
        raise KeyError(
            f"Unknown flow regime {flow_regime!r}. Configured regimes: {sorted(regimes)}"
        )
    return {key: float(factors.get(key, identity[key])) for key in identity}
...
def _alpha_for(
    ...
    *,
    ...
    """The angle of reach for this run.

    A user-supplied ``override_deg`` replaces the configured regional value and is
    clamped to the reviewed envelope, so a sensitivity run can explore the range the
    literature describes but cannot leave it. The flow-regime shift is applied on
    top: wet flow stops sooner (steeper alpha), powder runs further.
    """
    ...
    if override_deg is None:
        alpha = float(angles[release_size])
    else:
        bounds = config.require("runout.alpha_override_bounds_deg")
        alpha = float(
            min(max(float(override_deg), float(bounds["minimum"])), float(bounds["maximum"]))
        )
    return alpha + _flow_regime_factors(config, flow_regime)["alpha_shift_deg"]
...
def _start_point(zone: ReleaseZone, elevation: np.ma.MaskedArray) -> tuple[int, int, float]:
    ...
    elevation_values = np.asarray(elevation)
    candidates = zone.pixels & ~np.ma.getmaskarray(elevation) & np.isfinite(elevation_values)
    rows, cols = np.nonzero(candidates)
    if rows.size == 0:
        return 0, 0, float("-inf")
    elevations = elevation_values[rows, cols]
    best = int(np.argmax(elevations))
    return int(rows[best]), int(cols[best]), float(elevations[best])
...
def _friction_field(
    ...
    *,
    ...
    """
    forest_values = np.asarray(forest_mask.filled(np.nan), dtype="float32")
    plan = np.asarray(plan_curvature.filled(np.nan), dtype="float32")
    valid = (
        ~np.ma.getmaskarray(forest_mask)
        & ~np.ma.getmaskarray(plan_curvature)
        & np.isfinite(forest_values)
        & np.isfinite(plan)
    )
    forest = forest_values > 0.5
    channelled = plan < -0.3
    ...
    # Missing friction inputs are barriers, not neutral/open snow. NaNs make an
    # accidental future sample fail visibly even though the particle engine also
    # excludes these cells through ``valid``.
    # The flow-regime scales are applied after the terrain-dependent assignment, so
    # forest/gully structure is preserved and only the overall friction level moves.
    if mu_scale != 1.0:
        mu *= np.float32(mu_scale)
    if xi_scale != 1.0:
        xi *= np.float32(xi_scale)
    mu[~valid] = np.nan
    xi[~valid] = np.nan
    return mu, xi, valid
...
class FastRunoutEngine:
    ...
    def simulate(
        ...
        *,
        ...
        zone_cell_count = int(np.count_nonzero(zone.pixels))
        ...
        invalid_release_cells = zone_cell_count - len(release_cells)
        if release_cells.size == 0:
            return RunoutResult(
                ...
                warnings=[
                    "The release zone contained no cells with valid elevation; fast runout was "
                    "not simulated."
                ],
            )
        ...
        if maximum_flux > 0:
            ...
            denominator = np.log1p(maximum_flux)
            np.log1p(intensity, out=intensity)
            intensity /= denominator
        intensity *= envelope

        reached_count = int(np.count_nonzero(reached))
        envelope_count = int(np.count_nonzero(envelope))
        area = float(reached_count) * cell**2
        envelope_area = float(envelope_count) * cell**2
        reached_indices = np.argwhere(reached)
        horizontal_reach = (
            float(
                np.max(
                    np.hypot(
                        (reached_indices[:, 0] - start_row) * cell,
                        (reached_indices[:, 1] - start_col) * cell,
                    )
                )
            )
            if reached_count
        ...
        vertical_drop = start_z - float(np.min(z[reached])) if reached_count else 0.0
        ...
        metadata = {
            ...
            "horizontal_reach_m": round(horizontal_reach, 1),
            "vertical_drop_m": round(vertical_drop, 1),
            "method": (
            ...
            "calibration": (
                ...
                "avalanches, not values back-analysed from observed Mount Hosmer runouts. No "
                "eligible local runout-observation dataset is currently available to the project."
            ),
        ...
        return RunoutResult(
            ...
            warnings=(
                ...
                + (
                    [
                        f"{invalid_release_cells} release-zone cells lacked elevation coverage "
                        "and were excluded from fast-routing launch points."
                    ]
                    if invalid_release_cells
                    else []
                )
            ),
        )
...
class ParticleRunoutEngine:
    ...
    def simulate(
        ...
        *,
        ...
        regime = _flow_regime_factors(config, flow_regime)
        mu_field, xi_field, friction_valid = _friction_field(
            config,
            forest_mask,
            plan_curvature,
            mu_scale=regime["mu_scale"],
            xi_scale=regime["xi_scale"],
        )
        valid = elevation_valid & friction_valid
        ...
        if release_cells.size == 0:
            ...
            return RunoutResult(
                ...
                warnings=[
                    "The release zone contained no cells with complete elevation, forest, and "
                    "plan-curvature inputs; advanced runout was not simulated."
                ],
            )
        ...
        # Friction gaps are flow barriers, but the terrain gradient beside one must
        # still come from the known elevation surface. Filling elevation from the
        # combined mask would turn every forest-data gap into an artificial peak.
        filled = np.where(
            elevation_valid,
            z,
            np.nanmax(z[elevation_valid]) if elevation_valid.any() else 0.0,
        )
        ...
        for step in range(max_steps):
            ...
            # Fancy-index ``+=`` updates a repeated cell only once, even when many
            # particles occupy it.  Accumulate explicitly so the ensemble-density
            # field represents particles rather than unique cells per time step.
            np.add.at(visits, (r, c), 1.0)
        ...
        particle_cell_visits = int(visits.sum())
        maximum_particle_visits = float(visits.max())
        intensity = visits
        if maximum_particle_visits > 0:
            denominator = np.log1p(np.float32(maximum_particle_visits))
            np.log1p(intensity, out=intensity)
            intensity /= denominator
        ...
        #
        # This engine is a hybrid: Voellmy friction evolves particle velocity and
        # the per-particle empirical alpha energy line above supplies a stopping
        ...
        # Horizontal reach of the central footprint from the top of the start zone.
        # Measured exactly as the fast engine measures it so the two engines report
        # the same quantity under the same name -- the composite hazard index reads
        # it, and a term that changed meaning with the engine would be worthless.
        core_indices = np.argwhere(core)
        horizontal_reach = (
            float(
                np.max(
                    np.hypot(
                        (core_indices[:, 0] - start_row) * cell,
                        (core_indices[:, 1] - start_col) * cell,
                    )
                )
            )
        ...
        metadata = {
            ...
            "maximum_particle_visits_per_cell": maximum_particle_visits,
            ...
            "flow_regime_friction_scales": {
                ...
                "alpha_shift_deg": regime["alpha_shift_deg"],
            },
        ...
        if invalid_release_cells:
            warnings.append(
                f"{invalid_release_cells} release-zone cells lacked elevation, forest, or "
                "plan-curvature coverage and were excluded from particle launch points."
            )
