...
from ..hazard.conditions import Conditions
...
class AssessClient(Protocol):
    def __call__(
        self, conditions: Conditions, *, simulation_mode: str = "fast"
    ) -> dict[str, Any]: ...


class _RouteResult(BaseModel):
    intent: Literal["scenario", "question", "chat", "advice"]
    new_snow_cm: float | None = None
    wind_speed_kmh: float | None = None
    wind_direction_deg: float | None = None
    release_size: Literal["small", "medium", "large", "very_large"] | None = None


def _ollama_chat(system: str, user: str, *, temperature: float = 0.2, force_json: bool = False) -> str:
    """One typed, non-streaming turn through Ollama's official client."""
    try:
        response = Client(host=OLLAMA_URL, timeout=TIMEOUT_S).chat(
            model=OLLAMA_MODEL,
            messages=[{"role": "system", "content": system}, {"role": "user", "content": user}],
            stream=False,
            options={"temperature": temperature},
            format=_RouteResult.model_json_schema() if force_json else None,
        )
        ...
        if "not found" in exc.error.lower() or exc.status_code == 404:
            raise AssistantError(
                f"The model '{OLLAMA_MODEL}' is not installed in Ollama. Run: ollama pull {OLLAMA_MODEL}"
            ) from exc
        raise AssistantError(f"The local AI model returned an error: {exc.error}") from exc
...
def _summarize(assessment: dict[str, Any]) -> str:
    ...
    cond = assessment.get("conditions") or None
    zones = assessment.get("zones", []) or []
    runout = assessment.get("runout", {}) or {}
    release_index = assessment.get(
        "release_potential_index", assessment.get("hazard_score")
    )
    release_band = assessment.get(
        "release_potential_band", assessment.get("risk_level")
    )
    validation = assessment.get("validation", {}) or {}
    field_validation = validation.get("field_validation", {}) or {}
    release_coverage = (
        (assessment.get("coverage", {}) or {}).get("release_model", {}) or {}
    )
    scenario = assessment.get("scenario", {}) or {}
    if cond is None:
        conditions_line = (
            "Conditions: incomplete/unknown; no condition values were replaced with zero."
        )
    else:
        conditions_line = (
            f"Conditions: {cond.get('new_snow_cm')} cm new snow; wind "
            f"{cond.get('wind_speed_kmh')} km/h from {cond.get('wind_direction_compass')}; "
            f"release size {cond.get('release_size')}."
        )
    if release_index is None:
        release_line = (
            "Overall release-potential index: unavailable because supported current-condition "
            "inputs or coverage are incomplete; null is not a low score."
        )
    else:
        release_line = (
            f"Overall release-potential index: {release_index}/100 "
            f"(relative band: {release_band})."
        )
    core_area = runout.get("core_area_m2")
    runout_line = (
        "Simulated runout footprint: unavailable because no supported release result exists."
        if core_area is None
        else f"Simulated runout footprint: {round(float(core_area) / 1e6, 2)} km^2"
        + (f", peak modelled speed {runout.get('max_velocity_ms')} m/s." if runout.get("max_velocity_ms") else ".")
    )
    lines = [
        conditions_line,
        f"Scenario classification: {scenario.get('classification', 'not reported')}.",
        release_line,
        f"Release zones found: {assessment.get('release_zones', {}).get('zone_count', 0)}.",
        runout_line,
        "Runout uncertainty is a model-sensitivity envelope, not a statistical confidence interval.",
        f"Required release-input grid coverage: {release_coverage.get('valid_fraction', 'unknown')}.",
        f"Field-validation status: {field_validation.get('status', 'unavailable')}; eligible "
        f"observation count {field_validation.get('eligible_observation_count', 0)}. Software "
        "benchmarks are not field validation.",
    ]
    for zone in zones[:4]:
        lines.append(
...
_KNOWLEDGE = (
    ...
    "shed it), and a forest mask (trees anchor snow).\n"
    "- The hazard number is a RELATIVE index (0-100) with a band label -- NOT a probability and NOT "
    "calibrated.\n"
    "- Key limitation: no eligible local Mount Hosmer avalanche-observation dataset is currently "
    "available to this project, so nothing here is calibrated or field-validated against an "
    "observed event. It must never replace Avalanche Canada or field assessment."
)
...
def _base_conditions(assessment: dict[str, Any] | None) -> Conditions:
...
def _route(
    message: str, base: Conditions, history: list[dict[str, str]] | None
) -> tuple[str, Conditions, dict[str, Any]]:
    ...
    try:
        parsed = _RouteResult.model_validate_json(raw).model_dump(exclude_none=True)
